...
import com.example.kjh.nfc3.com.pro100svitlo.creditCardNfcReader.iso7816emv.impl.TagImpl;
...
public final class EmvTags {
    ...
    private static LinkedHashMap<ByteArrayWrapper, ITag> tags = new LinkedHashMap();

    static {
        for (Field f : EmvTags.class.getFields()) {
            if (f.getType() == ITag.class) {
                try {
                    addTag((ITag) f.get(null));
    ...
    public static ITag getNotNull(byte[] tagBytes) {
        ITag tag = find(tagBytes);
        if (tag == null) {
            return createUnknownTag(tagBytes);
        }
        return tag;
    }

    public static ITag createUnknownTag(byte[] tagBytes) {
        return new TagImpl(tagBytes, TagValueTypeEnum.BINARY, "[UNKNOWN TAG]", "");
    }

    public static ITag find(byte[] tagBytes) {
        return (ITag) tags.get(ByteArrayWrapper.wrapperAround(tagBytes));
    }

    private static void addTag(ITag tag) {
